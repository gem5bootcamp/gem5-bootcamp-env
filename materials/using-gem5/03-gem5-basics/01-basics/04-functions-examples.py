...
def function_1(input_list):
    n = len(input_list)
    for i in range(n):
        input_list[i] = 2 * input_list[i] # since parameters are passed by reference, the original list is also updated

def function_2(input_list):
    new_list = input_list[:] # making a copy of input_list and assigning the new list to new_list
    n = len(new_list)
    for i in range(n):
...
if __name__ == "__m5_main__":
    ...
    print("Output 1:", output1) # since function_1 does not have a return statement, the output would be None
    print()
